...
from tests.spark.databricks._types import paths
...
_EXPECTED_SCD2_SCHEMA = StructType(
    [
        ...
        # iteration 1's rows have no "newField" key at all (iteration 1
        # predates the column); iterations 2-11's rows always carry it
        # (string, or JSON null). Nullable, so a missing dict key converts
        # to NULL for iteration 1's rows --
...
def git_to_landing():
    ...
    for i in range(1, 12):
        iter_dir = f"iter{i}"
        DEFAULT_LOGGER.debug(f"copy json from git to landing ({iter_dir})")

        # tests/spark/fixtures/ -- shared with tests/spark/apache/
        # generate_fixtures.py, not owned by this (databricks) tier alone.
        from_dir = paths.tests.parent().joinpath("fixtures", iter_dir)
        to_dir = paths.landing.joinpath(iter_dir)
...
def landing_to_raw(iter: int | list[int]):
    ...
    for i in iter:
        iter_dir = f"iter{i}"
        DEFAULT_LOGGER.debug(f"copy parquet from landing to raw ({iter_dir})")

        landing = paths.landing.joinpath(iter_dir)
        for f in landing.walk():
            if str(f).endswith("parquet"):
                path = FileSharePath(f)

                for i in range(1, 4):
                    to_path = f.replace("landing", "raw").replace(iter_dir, "")
...
def create_expected_views():
    ...
    def _create_views(step: str, cdc: str):
        ...
        if step == "silver" and cdc == "scd2":
            # iter01.jsonl..iter11.jsonl are all committed NDJSON golden data.
            # iter02 onward were originally derived from a chained iter{N}.sql
            # of hand-authored VALUES rows unioned with iter{N-1}'s
            # non-current rows; that chain has since been generated and
            # committed as static .jsonl and the .sql sources retired (see
            # Task 3c). Loaded here in ascending iteration-number order; the
            # SQL loop below is a no-op for this dir since no .sql files
            # remain under it.
            for v in sorted(views.walk(file_format="jsonl")):
                DEFAULT_LOGGER.debug(f"create table {v}")
                iter_num = str(int(re.search(r"\d+", GitPath(v).get_file_name()).group()))
