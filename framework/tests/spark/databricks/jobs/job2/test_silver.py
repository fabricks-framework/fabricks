...
from tests.spark.databricks.compare import compare_silver_to_expected
...
def test_silver_princess_check():
    ...
    assert job.paths.to_commits.joinpath("0").exists(), "commit 0 not found"
    assert not job.paths.to_commits.joinpath("1").exists(), "commit 1 found"
