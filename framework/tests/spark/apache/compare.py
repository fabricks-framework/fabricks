"""Real-Spark (Apache-tier) equivalents of tests/spark/databricks/compare.py's
...
from tests.spark.databricks.compare import assert_dfs_equal
...
# -- it's a Databricks SQL extension. tests/spark/expected/silver/scd1/iter*.sql (the
...
_EXPECTED_SCD2_BASE_SCHEMA = StructType(
    [
        ...
        # table from this schema (seed_silver_table) writes the *type*
...
# introduces them). So a iters=[1]-only silver table genuinely has no
...
# this only mattered once this schema started feeding seed_silver_table.
...
def create_expected_views(spark: SparkSession, step: str, cdc: str) -> None:
    ...
    if step == "silver" and cdc == "scd2":
        ...
        # ... from expected.silver_scd2_iter{N-1} where not __is_current`
        ...
        # expected.silver_scd2_iter{2..9} entirely, since Task 9's
        # king_and_queen_built only calls create_expected_views for
        # ("silver", "scd2")/("silver", "scd1"), not per iteration. See Task 3's
        ...
        for ndjson_file in sorted(views_dir.glob("*.jsonl")):
            ...
            # "01") so it matches the un-padded table name (silver_scd2_iter1)
            # the scd1/iter*.sql oracle views reference. Same fix as
            # tests/spark/databricks/utils.py's create_expected_views (commit
            # d4f2498e) -- without it, "silver_scd2_iter01" is created but
            # "silver_scd2_iter1" (what iter1.sql selects from) is never found.
            ...
            df.write.mode("overwrite").saveAsTable(f"expected.silver_scd2_iter{iter_num}")
...
def load_expected(spark: SparkSession, cdc: str, iter: int) -> DataFrame:
    return spark.read.table(f"expected.silver_{cdc}_iter{iter}")
...
def load_expected_scd1_seed(spark: SparkSession, iter: int) -> DataFrame:
    ...
    an SCD1 table (see seed_silver_table), not for comparison.

    `expected.silver_scd1_iter{N}`'s own schema has no `__timestamp` column
    (tests/spark/expected/silver/scd1/iter{N}.sql: `select * except (__valid_from,
    __valid_to) from expected.silver_scd2_iter{N} qualify row_number() over
    (partition by id order by __valid_to desc) = 1`) — fine for *comparing*
    against a real table (compare_silver_to_expected only ever selects
    ...
    Derived from the same `expected.silver_scd2_iter{N}` root SCD1's own
    ...
    """
    scd2_df = spark.read.table(f"expected.silver_scd2_iter{iter}")
...
def seed_silver_table(table: Table, expected_df: DataFrame, keys: list[str]) -> None:
    """Seed a not-yet-existing Silver table directly from a prior iteration's known-
