...
class Table(DbObject):
    ...
    def _create(
        ...
        ddl_primary_keys = "-- no primary keys"
        ...
        if primary_keys:
            ddl_primary_keys = "constraint primary key (" + ", ".join(f"`{c}`" for c in primary_keys) + ")"

        if foreign_keys:
            ddl_foreign_keys = "constraint foreign key (" + ", ".join(f"`{c}`" for c in foreign_keys) + ")"
