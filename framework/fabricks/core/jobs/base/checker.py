...
class Checker(Generator):
    ...
    def _check(self, position: Literal["pre_run", "post_run"]):
        if self.options.check.get(position):
            DEFAULT_LOGGER.debug(f"check {position}", extra={"job": self})
    ...
    def check_post_run_extra(self):
        ...
        if min_rows or max_rows or count_must_equal:
            ...
            rows = df.collect()[0][0]
            if min_rows:
                DEFAULT_LOGGER.debug("check min rows", extra={"job": self})
                if rows < min_rows:
                    raise PostRunCheckException(f"min rows check failed ({rows} < {min_rows})", dataframe=df)

            if max_rows:
                DEFAULT_LOGGER.debug("check max rows", extra={"job": self})
                if rows > max_rows:
                    raise PostRunCheckException(f"max rows check failed ({rows} > {max_rows})", dataframe=df)

            if count_must_equal:
                DEFAULT_LOGGER.debug("check count must equal", extra={"job": self})
    ...
    def _check_duplicate_in_column(self, column: str):
        if column in self.table.columns:
            DEFAULT_LOGGER.debug(f"check duplicate in {column}", extra={"job": self})
            ...
            DEFAULT_LOGGER.debug(f"could not find {column}", extra={"job": self})
    ...
    def check_skip_run(self):
        if self.options.check.get("skip"):
            DEFAULT_LOGGER.debug("check if run should be skipped", extra={"job": self})
