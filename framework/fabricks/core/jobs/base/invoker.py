...
class Invoker(Checker):
    ...
    def _invoke_step(self, position: str, schedule: Optional[str] = None):
        ...
        if invokers:
            for i in invokers:
                DEFAULT_LOGGER.info(f"invoke by step ({position})", extra={"label": self})
                try:
                    notebook = i.get("notebook")
                    assert notebook, "notebook mandatory"
                    path = PATH_RUNTIME.joinpath(notebook)
    ...
    def extend_job(self, df: DataFrame) -> DataFrame:
        ...
        for e in extenders:
            name = e.get("extender")
            DEFAULT_LOGGER.info(f"extend ({name})", extra={"label": self})
    ...
    def extend_step(self, df: DataFrame) -> DataFrame:
        ...
        for e in extenders:
            name = e.get("extender")
            DEFAULT_LOGGER.info(f"extend by step ({name})", extra={"label": self})
