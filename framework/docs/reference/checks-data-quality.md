...
| `before`           | string (table/view) | unset   | no       | Assert the target table has fewer rows than a reference before the write. | Fail if condition not met.             |
| `after`            | string (table/view) | unset   | no       | Assert the target table has more rows than a reference after the write.   | Fail if condition not met.             |
...
Working scenarios are available in the sample runtime: [Sample runtime](../helpers/runtime.md)

Rich integration scenarios (repository layout): `framework/tests/integration/runtime/gold/gold/`
